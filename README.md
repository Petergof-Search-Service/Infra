...
| Файл | Назначение |
|------|------------|
| `frontend.conf.template` | Шаблон виртуального хоста. Директива `root` подставляется из секрета `REMOTE_PATH_FRONTEND` при деплое; в CI генерируется итоговый конфиг и выкатывается на сервер. |
...
| `REMOTE_PATH_FRONTEND` | Абсолютный путь к каталогу с собранным фронтендом на сервере (тот же путь, что и `REMOTE_PATH_FRONTEND` в репозитории Frontend). Пример: `/home/kizhinov/Petergof/frontend`. |
...
3. Workflow «Deploy infrastructure» (.github/workflows/deploy.yml) выполнится автоматически: подставит `REMOTE_PATH_FRONTEND` в шаблон, загрузит конфиг на сервер в `/etc/nginx/sites-available/`, включит его в `sites-enabled`, отключит дефолтный сайт nginx, проверит конфиг (`nginx -t`) и перезагрузит nginx.
...
- Каталог из `REMOTE_PATH_FRONTEND` существует и содержит собранный фронтенд (результат деплоя из репозитория Frontend).
