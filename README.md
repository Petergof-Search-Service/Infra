...
| Файл | Назначение |
|------|------------|
| `frontend.conf.template` | Шаблон виртуального хоста. Директива `root` подставляется из секрета `ROOT_PATH` при деплое; в CI генерируется итоговый конфиг и выкатывается на сервер. |
